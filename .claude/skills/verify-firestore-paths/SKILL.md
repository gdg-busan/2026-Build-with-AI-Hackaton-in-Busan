...
| `src/shared/api/client-actions.ts` | 클라이언트 API 호출 헬퍼 |
| `app/api/lookup/route.ts` | 코드 조회 API (서버사이드 Firestore 접근) |
| `src/features/*/ui/*.tsx` | Feature 컴포넌트 (클라이언트 Firestore onSnapshot 구독) |
| `src/features/*/model/*.ts` | Feature 훅 (클라이언트 Firestore 접근) |
| `src/widgets/team-detail/ui/*.tsx` | 팀 상세 위젯 (클라이언트 Firestore 접근) |
...
```bash
rg -n "import.*\{ db\b|import.*\{ auth\b" -g "*.tsx" -g "*.ts" app/ src/
```
...
```bash
rg -n "doc\(getFirebaseDb|collection\(getFirebaseDb" -g "*.ts" -g "*.tsx" app/ src/
```
...
```bash
rg -n "gdg-busan-2026" -g "*.ts" -g "*.tsx" app/ src/
```
